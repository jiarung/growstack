...
## 阻塞中（2026-09-07 盤點）→ **2026-09-11 全部清空**

B1、B3、B4 已解決；B2 降級為天花板（見下方「已知天花板」）——
它要的是量測活動而不是程式改動，留在這裡只會讓人以為還有東西可以修。
**兩個仍然開著、但不屬於本清單的問題**：通量比為什麼停在 3.3–3.7（B3 的前置，無 root cause），
以及 `kconsume` 對邊界前的歷史 cell 仍會套用舊 epoch 的值（B4 已揭露，未解）。
...
- **B2（原阻塞，2026-09-11 降級為天花板）— `k_adopted[bh1750_lux_main/daylight/diffuse] = 0.209968` 是已知錯誤值。**
...
修正後**仍有約 1.85× 殘差**。兩個 lux 桶的 CI 相對寬度都是 0.53（門檻 0.20），
所以它們**永遠不會 `valid`、永遠不會自動採納**，只能由 bootstrap 帶進來。

⚠ **2026-09-12 更正：天花板是真的，但這裡原本寫的原因是錯的。**

原文寫「`corr(k, 亮度) = +0.949` —— 感測器是壓縮響應」，並據此說要換模型形狀。
實測（`bh1750_lux_ref/daylight/diffuse`，n=13）：
...
`k = photone / sensor`，**k 與 photone 共用分子，隨機資料也會給出高相關** —— 那是比值
相關的假象，不是證據。對分母的 +0.444 才是有意義的，而它遠不到 0.949。

**而換模型形狀也不會有幫助**，這一點是量出來的。冪律擬合 `ln(photone) = c + γ·ln(sensor)`：

| 樣本集 | γ | R² | 殘差 |
|---|---|---|---|
| 去掉一筆離群（n=12） | 1.227 | 0.967 | 2.43× |
| 再去掉最暗那點 | 1.155 | 0.874 | 2.47× |
| **只留 sensor > 1000 的群（8/12 樣本在此）** | **0.749** | **0.189** | **2.36×** |

R²=0.967 完全來自橫跨三個數量級的動態範圍；在樣本真正聚集的區間裡 R² 只有 0.19、
γ 還翻到反方向，而且**殘差在每一種切法下都是約 2.4×，冪律一克都沒消掉**。

**散佈跟的是時刻，不是亮度**：`corr(ln k, 時刻) = −0.665`。那正是
[`PPFD-CAL-ROUTINE-PLAN.md`](../broker/PPFD-CAL-ROUTINE-PLAN.md) 開頭早就寫過的
「欄杆陰影隨太陽移動，散佈 2.9×」—— 手機和感測器沒有在看同一束光。
**沒有任何模型形狀能修好「兩個儀器在量不同的東西」。**

所以這個天花板的性質是**選址**，不是建模。同一顆感測器在燈下散佈只有 3%，
但 `bh1750_lux_ref` 刻意被遮蔽於燈之外 —— 遮蔽就是它存在的理由，所以它唯一能被
量測的條件，正好是散佈最大的那個。要改善得移動感測器或接受這個桶拿不到日光 k，
**不是換模型**。（換模型仍列在 `docs/photone-cal-pipeline.md` 的「明確不做(v1)」，
但現在的理由是「證據不支持」，而不只是「這版不做」。）
