...
`?rest=` 值得單獨說明,因為它同時解釋了一個看起來矛盾的觀測:**待機比串流還燙**
(`die_c` 70°C vs 54°C —— 兩個都是 S3 晶片,不是鏡頭)。原因是 `cameraInit` 用 `STILL_SIZE` 開 framebuffer 之後就沒再降
...
### 三個問題的答案(2026-09-05,上電實測)

上面那節立了三個問題,這裡是答案。先驗**揮發性**再談其餘:`0x3006` 上電後回到
`0xFF`(9/03 收工時留在 `0xEB`)—— 「寫入揮發、拔電即復原」是實測結論,不是推測,
這是在這個端點上做實驗安全的憑據。

**Q1 哪些時脈開著(0x3004–0x3006)**

| reg | 實測 | 廠商表 | 判讀 |
|---|---|---|---|
| 0x3004 | `0xFF` 全開 | `0xff` | 一致 |
| 0x3005 | `0xF7`(bit3 = 0) | **沒有這一格** | **open** —— 誰關的、關掉什麼,無來源可引 |
| 0x3006 | `0xFF` 全開 | `0xc3` | 已驗證不能照抄,可關的是 `0xEB` |

**Q2 MIPI 關了沒(0x300e)—— 結案**

    0x300E  0x58  01011000   MIPI power down / DVP enable

**與廠商表的 `{0x300e, 0x58}` 完全相同**:esp32-camera 早就把 MIPI PHY 關掉了。
「DVP 板上 MIPI PHY 還在燒電」這個過熱嫌疑**不成立,從清單上劃掉**。負面結果,
但省掉的是之後花在錯方向的時間。

**Q3 待機前後哪些 bit 真的變了(0x3008)**

    awake    0x02  00000010
    standby  0x42  01000010      0x02 ^ 0x42 = 0x40,只有 bit6
    awake    0x02  00000010

mask `0x40` 的寫法確認正確。附一個誠實的註腳:低位實際就是 `0x02`,和舊的整 byte
寫法 `0x42`/`0x02` 恰好相同 —— 舊寫法在**這顆**板子上沒造成傷害,但那是巧合不是
保證,改成 mask 仍然是對的。

**意外收穫:待機不會關時脈。** 待機中 `0x3004` / `0x3005` / `0x3006` / `0x300e`
一個 bit 都沒變。所以 `cam=idle` 和「關時脈域」是**兩個正交的手段,可以疊加** ——
`0x3006 = 0xEB` 不是 `cam=idle` 的子集,它有獨立價值。

待機 → 喚醒 → `/capture` 回 162282 bytes / 2560×1920 / `FFD9` 完整,來回沒有破壞
pipeline。

**一個 die 側的初步觀察,不是鏡頭的答案**:這次是 `rest=vga` 的第一次乾淨開機,
466s 時 `die_c 37.6 / die_max_c 37.6`、`ta 33.0` —— die 只比環境高約 4.6°C,對照
舊的待機 `die_c` 70°C。但 soak 只有 8 分鐘、期間做過 capture(會升到 QSXGA),而且
**die 從來就不是這條線要問的東西**:它早在 8/31 就被證實沒事(19 分鐘串流收斂
54°C 不失控)。

### 鏡頭溫度:量不到,而且擋點是物理的(2026-09-05)

`rest=vga` 到底幫 OV5640 降了幾度,**目前無法回答**,而且原因不在軟體。踩過的坑
記在這裡免得再踩一次:曾經開了一輪 `rest=qsxga` vs `rest=vga` 的 20 分鐘 A/B,量的
是 `die_c` —— 那是 S3,量了也只是把一個已知的答案再測一次。本節開頭那張表列得很
清楚:**鏡頭不在能量到的溫度裡**,`sensor.h` 根本沒有 temperature op。

擋點:MLX90640 **掛在同一顆頭上,看不到自己**。實測現況一幀是

    seq 3598   23.43..34.26 C   hot @ r1 c25   Ta 33.78 C

全幀最高 34°C —— 它對著房間,不是對著板子。要往下走只有兩條路,都是治具決定:

- **A.** 把 MLX90640 拉出來反過來對著 s3cam 板(飛線 10–20 cm;FOV 55°×35° 在
  15 cm 涵蓋約 15×9 cm,4 cm 見方的板子佔畫面剛好)
- **B.** 用第二支熱像儀對著板子,MLX 留在原位

對準之後的順序(軟體都已就緒,`--roi` / `--log` 在 62e0bed):

1. **先確認畫面裡哪一塊是誰**,不要用眼睛猜:`--watch` 開著,切一個只加熱特定
   元件的旋鈕 —— `?rest=qsxga` 只加熱 OV5640,`?cpu=240` 只加熱 S3,跟著變燙的
   座標就是它。
2. **測量點貼一小塊霧面膠帶**(Kapton / 黑電工膠帶)。模組金屬殼與裸 PCB 放射率
   低,熱像儀會嚴重低估;而我們要答的「有沒有超過 60°C 規格」是**絕對值**問題,
   放射率誤差會直接決定答案對錯。
3. 然後才是 A/B:`--roi` 框住那塊、`--log` 各記 20 分鐘,qsxga 先 vga 後
   (vga 從熱板起跑,勝幅被低估,是保守方向)。

在第 1 步完成之前,這條線一步都動不了。

## AF:韌體跑起來了,鏡頭沒有(2026-09-08)

AF 版鏡頭到手,blob 上傳做完了。結論分兩半:**韌體側完全成功,致動器側沒有反應。**
路上踩到一個會靜默毀掉整件事的陷阱,記在最前面。

### ⚠️ blob 是 4077 bytes,不是 4332 —— 那個數字是抽取 bug

本檔先前寫「4332 bytes」。**錯的**,而且錯法很陰險。rt-thread 的 header 每行結尾帶
位址註解:

    0x02, 0x0f, 0xd6, ... 0xe5, 0x1f, //0x8000,
    0x70, 0x72, 0xf5, ... 0xf6, 0x08, //0x8010,

用 `0x[0-9a-f]{2}` 掃這段文字,會把註解裡 `0x8000` 的前兩位 `80` 當成一個資料位元組
吃進去 —— **每 16 bytes 摻一個假的**,255 行就是 255 個假位元組,4077 變成 4332。

這種壞法不會有任何症狀告訴你資料錯了:4332 次 SCCB 寫入**全部被 ACK**,回讀也**完全
自洽**(寫什麼讀回什麼),韌體就是不跑,而所有可見的指標都說一切正常。
**解析前先剝註解,而且拿第二個來源對長度。**

### 兩個來源互相驗證

    ~/rt-thread/bsp/k210/driver/camera/ov5640af.h              OV5640_AF_Config[]
    espressif/esp32-camera  sensors/private_include/ov5640_af_firmware.h

**兩份逐位元組完全相同**,4077 bytes,sha256 `439245623bc99f3b...`。上游的
`sensors/ov5640_af.c` 執行的流程也與 `drv_ov5640.c` 的 `OV5640_Focus_Init()` 相同 ——
同樣的暫存器、同樣的順序。單一來源的 blob 只能信到「它長這樣」,兩個獨立來源一致才
能信到「它是對的」。

(Arduino core 2.0.17 內建的 esp32-camera **沒有** `set_focus_simple`,連 focus 符號
都沒有,所以這條路只能自己實作。)

### 流程(`src/s3cam/af.{h,cpp}`,端點 `GET /cam/af`)

    讀 0x3000 → 只設 bit5(MCU reset),其餘位元原樣保留
    4077 bytes 逐位元組寫到 0x8000..0x8FEC
    全部回讀比對          ← 參考驅動沒有這步,而它是唯一抓得到上面那個 bug 的東西
    清 0x3022..0x3028、0x3029 = 0x7F
    清 0x3000 bit5(放開 MCU)
    輪詢 0x3029 直到 0x70

兩處刻意與參考驅動不同:

- **`0x3000` 用 masked write。** 參考驅動整 byte 寫 `0x20`,在它自己的 init 流程裡
  沒事(其餘位元本來就是 0),但我們載入時相機**正在跑** —— 這是我們修過的 `0x3008`
  整 byte 寫入的同一個形狀。另外 bit6(程式記憶體 reset)若為 1 就直接拒絕上傳,
  否則 4332 次寫入全部落空,而失敗會在三步之後才以「韌體沒 ready」現身。
- **回讀驗證。** `write_fails` 數的是 I2C ACK,**被 ACK 不等於位元組存在那裡**。

### 這顆的程式記憶體只有 4095 bytes(實測)

`0x8FFF` 以上**全部別名到同一格**:只寫 `0x11` 到 `0x9000`,`0x8FFF`/`0x9001`/`0x90EB`
會一起變成 `0x11`,而 `0x8FFE` 不受影響。可用視窗 = `0x8000..0x8FFE`。

4077 bytes 結束在 `0x8FEC`,**塞得下,還剩 18 bytes**。(那份 4332 的假 blob 塞不下,
一度讓我以為記憶體大小就是根因 —— 別名是真的,但它從來不是原因。)

### 韌體確實在執行

| 證據 | 值 |
|---|---|
| 載入後 `0x3029` | `0x70` ready —— 我種進去的是 `0x7F`,**`0x70` 是 MCU 自己寫的** |
| `cmd 0x03` | `0x3029` → `0x10` |
| `cmd 0x04` | `0x3029` → `0x20` |
| `cmd 0x07` | `0x3029` → `0x20` |
| `0x3023` | 每次都回到 `0x00`(指令被消化) |

指令交握:`0x3023=0x01` → `0x3022=<cmd>` → 輪詢 `0x3023` 直到 `0x00`。
`0x03`/`0x04` 有出處(`OV5640_Auto_Focus()` 用 `0x08` 再 `0x04`);`0x07` 是用
`?cmd=` 逃生口試出來的,**未經文件證實**。
...
| 測試 | 結果 |
|---|---|
| 連續對焦 30 秒(`0x3029` 持續 `0x20` 搜尋中),期間連拍 4 張 | Laplacian 17.8 / 17.8 / 17.9 / 17.8 —— **平線,變動 0.6%** |
| 目標 @ 120mm(VL53L0X 實測),單次對焦前後 | 17.8 → 17.7 |
| 同一顆鏡頭 @ 1–3m | **銳利**,牆上電器的小數字讀得出來,紋理區 ~105 |

搜尋中的 VCM 必然掃過行程,銳利度必然大幅波動。**遠清、近糊、搜尋期間完全不動** ——
鏡頭停在遠焦位置沒有被驅動。

**量銳利度的陷阱**:`focus_score.py` 只取中央 50%。第一次量到 44 分讓人以為是糊的,
其實那張**是清楚的** —— 中央剛好是一片平牆加暗部。換成有紋理的區域是 105。
**分數低要先看畫面,再下結論。**

### 未解

VCM 為何不動:沒接上/沒供電、模組沒把 OV5640 的 AF 驅動腳拉出來、或金屬罐裡是固定
鏡片。軟體側已無路可走(上游做的事和我們一樣)。下一步是物理檢查:下指令時聽有沒有
VCM 的喀噠聲。

**另一個與對焦無關的發現:畫面是左右鏡像的**(拍到的字反了)。感測器 hmirror 設定,
之後做 registration 會咬人,單獨處理。
